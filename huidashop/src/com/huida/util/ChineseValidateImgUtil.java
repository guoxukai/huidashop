 ...
 *
 */
public class ChineseValidateImgUtil {
	 ...
	 * @throws IOException 
	 */
	public static void genImgStream(OutputStream os,HttpSession session) {
		// 设置 备选随机汉字的个数
		...
		// 在图片背景上增加噪点，增加图片分析难度
		g.setColor(getRandColor(random, 0, 199));
		...
		// 保存生成的汉字字符串
		String sRand = "";
		int smalIndex = random.nextInt(3);
		for (int i = 0; i < 4; i++) {
			...
			// 设置字体格式
			if(i==smalIndex){
				g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],Font.BOLD, 13));
			}else{
		...
		try{
			g.dispose();
		/*	File dFile  = new File("D:/f.jpeg");
			if(!dFile.exists()){
				dFile.createNewFile();
			}
			FileOutputStream fos  = new FileOutputStream(dFile);*/
			session.setAttribute("img_validate_code", sRand);
...
//		System.out.println(genImgStream(fos));
//		fos.close();
	}
	...
	public static boolean validate(String code,HttpSession session){
		String valcode = (String)session.getAttribute("img_valicode");
